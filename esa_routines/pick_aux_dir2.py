    ...
    for t in tryno_range:
        ...
        if len(file_list) == 0:
            ...
            if len(verified_file_list) > 0:
                ...
                diffs     = map(lambda x:     (x - ref_date).total_seconds() , dates)
                diffs_abs = map(lambda x: abs((x - ref_date).total_seconds()), dates)
                # logger.info("Now grep nearest date")
                min_val = min(diffs_abs)
                min_idx = diffs_abs.index(min_val)
                if diffs[min_idx] < 0:
                    result1 = verified_file_list[min_idx]
                    result2 = verified_file_list[min_idx + 1]
                else:
                    result1 = verified_file_list[min_idx - 1]
                    result2 = verified_file_list[min_idx]
